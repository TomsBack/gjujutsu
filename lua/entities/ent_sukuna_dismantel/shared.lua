...
function ENT:Initialize()
    ...
    if SERVER then
		...
		local angp = self.Owner:GetAngles()
		local bone = "ValveBiped.Bip01_R_Finger12"
		self:SetPos(self.Owner:WorldSpaceCenter() + penis2 * 30 + penis1 * -400 + penis3 * 100)--+ penis1 * 5 + penis2 * 40)
        self:SetIdealPos(self.Owner:WorldSpaceCenter() + penis2 * 30 + penis1 * -400 + penis3 * 100)--+ penis1 * 5 + penis2 * 40)

		timer.Create("MovePurple222"..SteamIDForTimer, 0, 1, function()
			if IsValid(self) and IsValid(self.Owner) then
				local tr = self.Owner:GetEyeTrace()
				local dir = (tr.HitPos - self.Owner:EyePos()):GetNormalized()
				timer.Create("MoveForward2"..SteamIDForTimer, 0, 0, function()
					if IsValid(self) and IsValid(self.Owner) then
						local speed = 50
					...
					if IsValid(self) then
						if IsValid(tr1.Entity) then
							if tr1.Entity:IsValid() and ( tr1.Entity ~= self.Owner and tr1.Entity ~= self ) and ( tr1.Entity:IsRagdoll() or tr1.Entity:IsNPC() or tr1.Entity:IsNextBot() or tr1.Entity:IsPlayer() or tr1.Entity:GetClass() == "prop_physics") then
								tr1.Entity:EmitSound(Sound('limitless/teleport.mp3'))
								if tr1.Entity:IsNPC() then
									local oldvel = tr1.Entity:GetVelocity()
									tr1.Entity:TakeDamage(300)
							...
							if tr1.Entity:IsValid() and ( tr1.Entity ~= self.Owner and tr1.Entity ~= self ) and (tr1.Entity:GetClass() == "ent_sukuna_domain_expansion" or tr1.Entity:GetClass() == "ent_sukuna_domain_expansion3") then
								tr1.Entity:TakeDamage(math.random(100,500))
							end
						end
					end
				end)
		...
		timer.Create("DeletePurpl2e"..SteamIDForTimer,5,1, function()
			if IsValid(self) then
				self:Remove()
				timer.Remove("MoveForward2"..SteamIDForTimer)
				timer.Remove("MovePurple222"..SteamIDForTimer)
				timer.Remove("DeletePurpl2e"..SteamIDForTimer)
			end
		end)
	end
end
