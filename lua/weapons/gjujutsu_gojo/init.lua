...
hook.Add("EntityTakeDamage", "gJujutsu_InfinityNoDamage", function(ply, dmg)
	...
	if weapon:Gjujutsu_IsGojo() and weapon:GetInfinity() then
		if attacker:IsValid() and weapon.DamageBypassBase[attacker.Base] then return end
		if inflictor:IsValid() and weapon.DamageBypassBase[inflictor.Base] then return end
		if attacker:IsValid() and weapon.DamageBypassEnts[attacker:GetClass()] then return end
		if inflictor:IsValid() and weapon.DamageBypassEnts[inflictor:GetClass()] then return end
