...
function SWEP:Slashes()
	...
	local owner = self:GetOwner()
	self:SetBusy(true)
	self:SetNextAbility3(CurTime() + 10)
	for k, v in pairs( ents.FindInSphere(self.Owner:GetPos(), 1000) ) do
        if SERVER and (IsValid(v) and v ~= self.Owner and v:IsPlayer() or v:IsNPC() or v:IsNextBot()) and v:Health() > 0 then
            v.slashed = ents.Create("ent_sukuna_slashs")
            v.slashed:SetOwner(v)
			v.slashed.sukuna = owner
